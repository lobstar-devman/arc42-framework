...
- **Implementation containers read docs over HTTP, not a bind-mount — and agents read the raw endpoint, not the rendered one.** The docs-toolkit serves two things, never a filesystem bind-mount: the rendered site (`mkdocs serve`, port 8000) for a human reading it in a browser, and `docs/` plus every sources-of-truth directory served as plain static files (port 8001, see `docker-compose.yml`'s `architecting-toolkit-raw` service and `serve-raw.py`) for an implementation agent to fetch directly — the same paths, unrendered, with no HTML/nav/theme chrome to parse around. `/` on port 8001 redirects to `/docs/index.md`; toolkit-authoring files sitting directly at the workspace root (this file, `AGENTS.md`, `CLAUDE.md`, `mkdocs.yml`) are denied there — they're how the toolkit is authored, not architecture content for an implementer to consume. Don't bind-mount the docs repo's filesystem into an implementation container "for convenience" either way; reading it over HTTP keeps the implementation from being coupled to living in the same repo/filesystem as the docs.
