...
in this documentation: [Home](index.md), the
[Sources of Truth](sources-of-truth/index.md) pages, entries in the
[ADR log](decisions/index.md), and free-text sections inside arc42 pages
(for example, the cross-cutting-concerns text in
[Cross-cutting Concepts](arc42/08-crosscutting-concepts.md)). Without a
shared style, pages written by different authors — human or agent — drift
apart in tone, terms, and structure, which makes the site harder to trust
and slower to read.
...
**Out of scope:** generated content — BPMN/CMMN/DMN labels, OpenAPI
descriptions rendered by Redoc, Structurizr DSL element names, Mermaid
node text. Each of those follows its own source-of-truth format, and
authors should still write them clearly, but this guide governs prose
...
Keep to one idea per sentence, and one main idea per paragraph, with each
sentence following logically from the one before it.
...
phrase when a pronoun or synonym would do). A shorter sentence is easier
to scan and easier to translate.
...
motivation, then walk through the mechanics.
...
to see — a list without a lead sentence forces the reader to guess its
purpose. Use bullets for items with no required order, and numbers when
the reader must follow steps in sequence. Add a comma after an
introductory clause (as in this sentence) to mark where the main clause
begins.

Write link text that makes sense out of context — "see the
[ADR log](decisions/index.md)," not "see [here]." A screen reader user
often jumps from link to link without the surrounding sentence, so the
link text itself has to carry the destination. Give every embedded
...
references, since a large share of this site's readers — and its
implementation agents — are reading a second language or no natural
language at all.

## Proofread before publishing

Draft, step away, then reread with fresh eyes before you publish. On the
reread, check specifically for inconsistent tone, tense, and formatting,
and for near-miss word swaps — "he" for "the," for instance — that a
...
you edit into line with this guide rather than leaving the drift in
place. The decision to adopt this guide is recorded as
